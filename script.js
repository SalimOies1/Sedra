...
for (let i = 0; i < 20; i++) {
    ...
    p.style.setProperty('--petal-x', '0px');
    petalContainer.appendChild(p);
    petals.push(p);
}

/* ================= تفاعل الزهور مع اللمس وهز الموبايل ================= */
...
let lastTouchEffect = 0;

function affectPetalsAtPoint(x, y) {
    const now = Date.now();
    if (now - lastTouchEffect < 70) return;
    lastTouchEffect = now;
    ...
    petals.forEach(p => {
        ...
        const distance = Math.hypot(petalX - x, petalY - y);

        if (distance < 100 && !p.classList.contains('touched')) {
            affected = true;
            p.classList.add('touched');

            setTimeout(() => {
                p.classList.remove('touched');
            }, 900);
        }
    });

    if (affected) createTouchHeart(x, y);
}
...
window.addEventListener('touchstart', event => {
    ...
    if (touch) affectPetalsAtPoint(touch.clientX, touch.clientY);
}, { passive: true });
...
function startMotionListener() {
    ...
    window.addEventListener('devicemotion', event => {
        ...
        const x = Math.max(-35, Math.min(35, (acceleration.x || 0) * 3));
        const y = Math.max(-18, Math.min(18, (acceleration.y || 0) * 1.5));

        petals.forEach(p => {
            p.style.setProperty('--petal-x', x + 'px');
            p.style.setProperty('--petal-y', y + 'px');
        });
    }, { passive: true });
}
...
/* ================= تفاعلات إضافية: لمسة الزهور + الهزة ================= */
...
function handlePetalTouch(x, y) {
    ...
    allPetals.forEach(p => {
        ...
        if (dist < radius && p.style.opacity !== '0') {
            p.style.opacity = '0';
            setTimeout(() => {
                p.style.opacity = '0.6';
            }, 1200);
...
/* --- الزهور تتمايل مع اتجاه هز الموبايل --- */

let lastAccX = null, lastAccY = null, lastShakeTime = 0;
let shakeResetTimer = null;

function reactToShake(deltaX, deltaY) {
    const offsetX = Math.max(-30, Math.min(30, deltaX * 2));

    allPetals.forEach(p => {
        p.style.left = `calc(${p.dataset.baseLeft} + ${offsetX}px)`;
    });

    clearTimeout(shakeResetTimer);
    shakeResetTimer = setTimeout(() => {
        allPetals.forEach(p => {
            p.style.left = p.dataset.baseLeft;
        });
    }, 500);
}
...
function handleMotion(event) {
    ...
    const deltaX = (lastAccX === null) ? 0 : acc.x - lastAccX;
    const deltaY = (lastAccY === null) ? 0 : acc.y - lastAccY;
    const force = Math.hypot(deltaX, deltaY);

    if (force > 12) {
        reactToShake(deltaX, deltaY);
        lastShakeTime = now;
    }
...
if (window.DeviceMotionEvent) {
    if (typeof DeviceMotionEvent.requestPermission === 'function') {
        document.addEventListener('touchend', function requestMotionPermission() {
